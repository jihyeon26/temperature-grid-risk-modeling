...
### Mean and distribution grid stress across temperature bins
```{r fig-price, fig.cap="Figure 6. Mean and distribution of secondary control energy prices across temperature bins.", fig.show='hold', out.width='48%'}

# Plot 1: Average Grid Stress
df %>%
  group_by(temp_bin) %>%
  summarise(
...
# Plot 2: Boxplot (Price Distribution)
ggplot(df,
       aes(x = temp_bin,
           y = sec_energy_price,
           fill = temp_bin)) +
  geom_boxplot(width = 0.7, show.legend = FALSE, outlier.size = 0.7) +
  theme_bw(base_size = 14) +
  theme(panel.border = element_blank(),
        axis.line = element_line(colour = "grey80")) +
...
### Relationship between grid stress and price of secondary control energy
```{r fig-sec-energy-vs-costs, fig.cap="Figure 7. Relationship between secondary control energy deployment and balancing prices, colored by year (2020–2024). Higher activation levels are associated with elevated and more volatile prices, with notable differences across years reflecting changing market conditions.", echo=FALSE}

# 1. Prepare the data: Extract year and ensure it's a factor (categorical)
df_plot <- df %>%
...
p <- ggplot(df_plot, 
            aes(x = sec_energy_mw, 
                y = sec_energy_price, 
                color = year)) + # Changed color to 'year'
  geom_point(size = 1, 
             alpha = 0.6,    # Added transparency to see overlapping points better
             aes(text = paste("Year:", year,
                              "<br>sec_energy_mw (MW):", round(sec_energy_mw, 2),
                              "<br>sec_energy_price (EUR):", round(sec_energy_price, 2)))) +
  scale_color_viridis_d() +  # Note the '_d' for discrete data (factors)
  theme_bw(base_size = 14) + # Larger text as we discussed before
...
The scatter plot shows how secondary control energy deployment relates to balancing prices, with observations colored by year. A positive relationship is visible: higher activation levels are generally associated with higher prices and greater price dispersion.

However, the strength of this relationship varies across years. In 2020, prices remain relatively low and stable even at moderate activation levels. In contrast, 2022 stands out with substantially higher prices and frequent extreme spikes, even when activation volumes are comparable to earlier years. The years 2023 and 2024 show elevated price levels relative to 2020, though less extreme than 2022.

This pattern indicates that while grid stress increases cost pressure, the magnitude of economic exposure depends on broader market conditions. Elevated activation raises the likelihood of high prices, but the intensity of price responses differs across market regimes.
...
In addition, the relationship between grid stress and balancing prices varies across years, indicating that economic exposure is influenced not only by activation levels but also by broader market conditions. Elevated stress increases the likelihood of high and volatile prices, but the magnitude of cost escalation depends on the prevailing market regime.
